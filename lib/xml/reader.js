...
(function() {
  var StringToSDMXStream, XMLReader, compactData_v2_0, genericData_v2_0, genericTimeSeriesData_v2_1, handlers, sax, structure_v2_0, structure_v2_1,
  ...
  XMLReader = (function(_super) {

    __extends(XMLReader, _super);

    XMLReader.name = 'XMLReader';

    function XMLReader(log, startDepth) {
      ...
      XMLReader.__super__.constructor.apply(this, arguments);
    }

    XMLReader.prototype.write = function(data) {
      this.parser.write(this.bufferToStr(data));
      return XMLReader.__super__.write.apply(this, arguments);
    };

    XMLReader.prototype.parseURN = function(urn) {
      var ref, result;
      ref = {};
    ...
    XMLReader.prototype.parseTag = function(name) {
      if (name.indexOf(':') !== -1) {
        return {
          prefix: name.split(':')[0],
    ...
    XMLReader.prototype.convertBool = function(attrs, attr) {
      if (attrs[attr] != null) {
        return attrs[attr] = attrs[attr] === 'true';
      }
    };

    XMLReader.prototype.currentNamespace = function(key, level) {
      if (level == null) {
    ...
    XMLReader.prototype.nameSpaceForElement = function(qualifiedName) {
      var key;
      key = 'xmlns' + (qualifiedName.prefix != null ? ':' + qualifiedName.prefix : '');
      return this.currentNamespace(key);
    };

    XMLReader.prototype.onRootElement = function(qualifiedName, tag) {
      var namespace;
    ...
    XMLReader.prototype.onopentag = function(tag) {
    ...
    XMLReader.prototype.onclosetag = function(tag) {
    ...
    XMLReader.prototype.ontext = function(chars) {
      return this.stringBuffer += chars.replace(/^\s+|\s+$/g, '');
    };

    XMLReader.prototype.onerror = function(msg) {
      this.emit('error', msg);
