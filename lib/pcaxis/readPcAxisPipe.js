...
(function() {
  var CONCEPT, LANGUAGE, MANDATORY, PCAXIS_KEYWORDS, PcAxisParser, ReadPcAxisPipe, cdc, cdcl, parseContact, parsePcAxisDate, pcxc, sdmx,
  ...
  PcAxisParser = require('./pcAxisParser').PcAxisParser;
  ...
  parsePcAxisDate = function(date) {
    var dd, hh, mm, mmm, yyyy;
    if (date == null) {
      return;
    }
    yyyy = date.slice(0, 4);
    mmm = date.slice(4, 6) - 1;
    dd = date.slice(6, 8);
    hh = date.slice(9, 11);
    mm = date.slice(12, 14);
    return new Date(yyyy, mmm, dd, hh, mm);
  };

  parseContact = function(contact) {
    return contact.replace(/#/g, ' ').match(/\S+@\S+/)[0];
  };

  ReadPcAxisPipe = (function(_super) {
    ...
    ReadPcAxisPipe.prototype.emitConceptScheme = function() {
      ...
      this.emitSDMX(sdmx.CONCEPT_SCHEME, cdc.CROSS_DOMAIN_CONCEPTS);
      return this.emitSDMX(sdmx.CONCEPT_SCHEME, pcxc.PC_AXIS_CONCEPTS);
    };

    ReadPcAxisPipe.prototype.emitDataStructureDefinition = function() {
      var conceptId, dim, dimension, domain, dsd, i, key, value, _i, _j, _len, _len1, _ref, _ref1, _ref2;
      dsd = {
        id: this.dataSetId,
        agencyID: this.agencyId,
        version: '1.0',
    ...
    ReadPcAxisPipe.prototype.emitAttributes = function() {
      var attribute, key, keyword, _ref;
      attribute = {
        attributes: {}
      };
      _ref = this.keywords;
      for (key in _ref) {
        keyword = _ref[key];
        if (PCAXIS_KEYWORDS[key][CONCEPT] == null) {
          continue;
        }
        attribute.attributes[PCAXIS_KEYWORDS[key][CONCEPT]] = keyword[0].value;
      }
      return this.emitSDMX(sdmx.DATA_SET_ATTRIBUTES, attribute);
    };
