...
exitActions =
	...
	'Header/DataSetAction': ->
		switch @stringBuffer
			when 'Update' then headerCur.dataSetAction = 'Append'
			else headerCur.dataSetAction = @stringBuffer
	'Header/Extracted': ->
		headerCur.extracted = util.xmlDateToJavascriptDate @stringBuffer
	'Header/ReportingBegin': ->
		headerCur.reportingBegin = @stringBuffer
	'Header/ReportingEnd': ->
		headerCur.reportingEnd = @stringBuffer
	'Header/Source': (attrs) ->
		attrs['xml:lang'] ?= 'en'
		headerCur.source ?= {}
