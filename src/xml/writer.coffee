...
class XMLWriter extends SDMXToStringStream
	constructor: (log) ->
		@header = {}
		@rootElemName = ''
		@doc = builder.create()
	...
	afterLast: (type) ->
		switch type
			when 'codelist' then '</Codelists>\n'
			when 'conceptScheme' then '</Concepts>\n'
			when 'dataStructure' then '</DataStructures>\n'
			else ''
